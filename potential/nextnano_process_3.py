...
def reshapePotential(potentialL, voltages):
    """
    ...
    for i in potentialL:
        ...
        potential3D = slicePotential3D(i[1], i[2][0], i[2][1], i[2][2])
        i[1] = potential3D
    ...
    # for v in voltages:
    #     shape = shape + (len(v),)
    shape = (5, 103, 35, 78)       # TODO: generalize by adding coord 
...
def interp(potential, voltages, coord):
    ...
    z = [float(zi) for zi in coord[2]]
    # variables = tuple(voltages) + (x, y, z)
    variables = (voltages[3], x, y, z)
...
potentialND = reshapePotential(potentialL, voltages)
out = interp(potentialND, voltages, coord)
print(out([0.2,1.0,1.0,-1.0]))